...
#if YM_DEBUG_LEVEL > 1
			fprintf(debug_file, "CHANNEL[%d], SLOT[%d] DR = %.2X  EincD = %.6X\n", nch, nsl, data, SL->EincD);
#endif
			break;
...
//			if (data &= 0xF) SL->RR = &DR_TAB[data << 2];
//			else SL->RR = &NULL_RATE[0];
...
int YM2612_Init(int Clock, int Rate, int Interpolation)
{
	...
	// 144 = 6 * 24 = 12^12 (this is a good number)
...
//				if ((GYM_Dumping) && ((YM2612.OPNAadr == 0x22) || (YM2612.OPNAadr == 0x27) || (YM2612.OPNAadr == 0x28))) Update_GYM_Dump(1, YM2612.OPNAadr, data);
		...
		case 3:
#if YM_DEBUG_LEVEL > 1
			fprintf(debug_file, "Reg[%.3X] = %.2X\n", YM2612.OPNBadr + 0x100, data);
#endif
