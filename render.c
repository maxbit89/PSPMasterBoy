...
void pspSetObjRenderingRoutine()		{
	if (menuConfig.video.spriteLimit)
	    render_obj = render_obj_sms;
	else
	    render_obj = render_obj_sms_nospritelimit;
}
...
void render_line(int line)
{
    ...
    else
    {
        ...
        if(vdp.reg[0] & 0x20)
        {
			//Only disable it if the left bar is expressively shown
			if (menuConfig.video.leftBar == 1)
...
/* Draw sprites without sprite limit */
