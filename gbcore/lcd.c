...
void lcd_sprite_render_color(void *buf,int scanline)
{
	...
	for (i=39;i>=0;i--){
		...
		x=oam[(i<<2)+1]-8;
		if (x<=-8||x>SIZE_LINE_CLIP) continue;					//横方向の範囲判定
...
//		n_mpal16 = PAL_SGB;
		...
		//Pour le SGB comme on fera plusieurs acc鑚 (dont un r/w dans lcd_sgb_render), le cache vaut la peine
