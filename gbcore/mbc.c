...
//		fprintf(file,"external read [%04X]\n",adr);
	...
	case 0x22: // コロコロカービィ
		...
		case 0xA010:
			return 0;
		case 0xA020:
			return gbe_getSensor(true)&0xff;
		case 0xA030:
			return (gbe_getSensor(true)>>8)&0xf;
		case 0xA040:
			return gbe_getSensor(false)&0xff;
		case 0xA050:
			return (gbe_getSensor(false)>>8)&0xf;
		case 0xA060:
			return 0;
		case 0xA070:
			return 0;
...
void mbc_mbc3_write(word adr,byte dat)
{
	...
	case 3://RTCラッチ
		if (dat==0){ // Latchはずす
			...
			if (!mbc3_latch){
				mbc3_sec=gbe_getTime(8);
				mbc3_min=gbe_getTime(9);
				mbc3_hour=gbe_getTime(10);
				mbc3_dayl=gbe_getTime(11);
				mbc3_dayh=gbe_getTime(12);
			}
			mbc3_latch=1;
		}
